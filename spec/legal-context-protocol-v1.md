...
**Status:** Draft — released for community review
**License:** Apache 2.0
...
LCP is a foundational standard, not an implementation specification. The normative core (Section 2) defines the complete conformance surface; Sections 3 through 10 are advisory guidance on progressively stronger evidentiary practice; the appendices are informative illustrations of how implementations may realize that guidance. Nothing outside Section 2 is required for conformance.
...
The legal terms referenced in `legal-context.json` MUST be a standalone file — a discrete, downloadable artifact. Not a section of a webpage. Not dynamically rendered HTML. The file format does not matter for conformance: Markdown, JSON, plain text, PDF, Word, or any other format. The file being a self-contained, downloadable artifact is the requirement.

Terms intended for consumption by autonomous agents SHOULD be published in a machine-readable, text-based format — Markdown, JSON, or plain text. Page-layout and word-processor formats such as PDF and Word are not recommended for agent-facing terms, because reliable automated extraction cannot be guaranteed; they remain appropriate for human- or counsel-facing terms and for interoperability with documents that already exist in those formats (see Section 2.8).
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `termsFormat` | string | OPTIONAL | The format or schema of the terms document. Signals to agents whether the terms are machine-readable before fetching. Known values: `"markdown"`, `"json"`, `"plain"`, `"html"`, `"pdf"`. Terms intended for agent consumption SHOULD use a machine-readable value (`"markdown"`, `"json"`, or `"plain"`); `"pdf"` is not recommended for agent-facing terms (see Section 2.8). Implementations MAY define additional values for application-specific structured formats. When absent, agents determine format from the `Content-Type` header after fetching. See Section 2.8 for tradeoffs among these formats. |
| `atrHash` | string | OPTIONAL | SHA-256 hash of the **Agentic Transaction Record** (ATR) — the ATR is the terms document itself (the standalone artifact of Section 2.3), and `atrHash` is its digest. `0x`-prefixed hex (66 characters); lowercase hex is RECOMMENDED for emission. Two `atrHash` values are equal when their decoded 32-byte values are equal, so implementations MUST compare the decoded bytes rather than the strings. See [Level 2](#level-2-provable). |
...
The standard does not prescribe a format for the terms document. Section 2.3 requires only that the document be a standalone, downloadable artifact. This subsection summarizes the tradeoffs among common formats, listed in rough order of suitability for autonomous-agent consumption. For agent-facing terms, the machine-readable text formats (Markdown, JSON, plain text) are recommended; page-layout and word-processor formats (PDF, Word) are not.

**Markdown.** Readable by both humans and agents, straightforward to author, diff-friendly under version control, low tooling cost. The recommended general-purpose default for agent-facing terms.
...
**PDF.** Not recommended for agent-facing terms. PDF is a page-layout format: text is positioned visually rather than stored as a clean linear sequence, extraction order is not guaranteed, and scanned or image-based PDFs may carry no extractable text at all — so an agent cannot reliably parse the terms it is being asked to commit to. PDF carries no integrity guarantee of its own; integrity in this standard is provided by `atrHash` (Section 2.5, Level 2), which applies equally to any format. PDF's legitimate role is human- and counsel-facing review — it remains the drafting and filing format of legal practice — and interoperability with terms that already exist as counsel-drafted PDFs. Where terms originate as a PDF but will also be consumed by agents, publishing a machine-readable companion (Markdown, JSON, or plain text) carrying the same substance is recommended.

**Word and other word-processor formats.** The same limitations as PDF apply: proprietary, layout-oriented structure makes reliable automated extraction unsafe. Not recommended for agent-facing terms; suitable only for human- or counsel-facing review or interoperability with pre-existing documents.
...
An agent can find the published terms and conditions of a service. The terms are readable at a known URL. Proceeding with a transaction after discovering the terms constitutes implicit consent — the browsewrap pattern. Its electronic-form validity rests on medium-neutrality under U.S. law (the Electronic Signatures in Global and National Commerce Act (E-SIGN, 15 U.S.C. § 7001 et seq.) and the Uniform Electronic Transactions Act (UETA § 7), with UETA § 14 specifically validating contracts formed by automated electronic agents) and EU law (the E-Commerce Directive 2000/31/EC and eIDAS Regulation (EU) No 910/2014); whether the terms were presented conspicuously enough to bind is governed by case law and varies by jurisdiction.
...
- **Format requirements** — the formats the buyer's parser supports, and the formats it will accept as agent-facing terms. A buyer with a JSON-only parser may decline to transact with services that publish only PDF terms; more generally, an agentic deployment SHOULD prefer machine-readable, text-based terms and MAY decline page-layout formats such as PDF — both because automated extraction is unreliable and because parsing prose from such formats widens the prompt-injection surface (Section 12.7).
...
### 8.3 Settlement Binding Patterns
...
- **Opaque Challenge Parameter** — the binding is committed to a signed challenge structure but never transmitted on-chain; no extra contract or transaction, at the cost of on-chain visibility.
- **Id-Reuse (Hashed Binding)** — the protocol's required binding derivation is executed with `atrHash` as an input; no new contract, but the on-chain value is a non-invertible hash.
...
- Where the host protocol requires the reused identifier to be unique per transaction, the ATR itself can be made unique per transaction — for example, by incorporating a date-time into the terms document at proposal time (Section 6.1) — so that each settlement binds a distinct `atrHash`.
...
Appropriate where the dispute forum accepts service-maintained records or where LCP adoption is informational only. Deployments that need stronger evidence should pair advisory HTTP metadata with one of the six settlement binding patterns above.
...
- Ryan, B., Moxey, J., Sproule, R., and Ragsdale, S., "Service Discovery for HTTP Payment Authentication", Internet-Draft draft-payment-discovery-00, June 2026. Published at paymentauth.org.
- OpenAI, Stripe, and Meta, "Agentic Commerce Protocol", version 2026-04-17.
...
- [EIP-712] Bloemen, R., Logvinov, L., and Evans, J., "Typed structured data hashing and signing", Ethereum Improvement Proposal 712, September 2017 (Final).
- [ERC-3009] Kim, P., Britz, K., and Knott, D., "Transfer With Authorization", Ethereum Request for Comments 3009, September 2020 (Draft; requires EIP-20 and EIP-712).
...
**What the buyer declares.** The buyer's policy now requires an explicit recourse path. The policy is cumulative — the Level 3 constraints (hash verification, signing thresholds, allowed signing keys) remain in force; only the additions are shown:
...
**Structural shape.** The seller advertises a value `V` derived from `atrHash` (either `V = atrHash` directly, or `V = atrHash[:N]` truncated to fit a fixed-width slot of N bytes). The advertised value rides in a proposal field the protocol treats as opaque, is carried through to the settlement transaction, and is emitted as an indexed topic in the resulting event.

**On-chain footprint.** No additional contracts. No additional transactions. Gas cost identical to the canonical settlement.

**Recovery.** An auditor reads the settlement transaction receipt, extracts the indexed topic, and recovers `V`. If `V = atrHash` directly, the binding is recovered. If `V` is a truncation, the auditor matches a candidate `atrHash` by truncating it; the prefix is deliberately public and serves as an audit-trail pointer, with full recovery completed by the parties' retained copies of the terms document.
...
**Structural shape.** Suppose the protocol requires an authorization nonce of the form `nonce = H(id || realm)`. The seller chooses `id = atrHash`. The on-chain settlement therefore carries `nonce = H(atrHash || realm)`. Where the protocol requires `id` to be unique per transaction, the ATR is made unique per transaction (see §8.3.5 and Section 6.1), so each settlement binds a distinct `atrHash`.
